...
export default async function request(endpoint, { method = 'GET', body = null, headers = {}, signal, isMultipart = false, skipRetry = false } = {}) {
  ...
  const token = localStorage.getItem('ezpay_token');
  ...
  while (attempt <= RETRY_CONFIG.maxRetries) {
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 503 && !skipRetry && shouldRetry(res.status, attempt, RETRY_CONFIG.maxRetries)) {
          ...
          if (authHandlers && authHandlers.handleAuthError && hadAuthToken && !isAuthEndpoint) {
            authHandlers.handleAuthError(res.status, 'El servicio no esta disponible despues de varios intentos. Tu sesion se cerrara por seguridad.', endpoint, hadAuthToken);
          }
        } else if ((res.status === 401 || res.status === 403) && !skipRetry) {
          if (authHandlers && authHandlers.handleAuthError && hadAuthToken && !isAuthEndpoint) {
            authHandlers.handleAuthError(res.status, data?.message || 'Sesion invalidada', endpoint, hadAuthToken);
          }
      ...
      if (error.name === 'TypeError' && error.message.includes('Failed to fetch')) {
        ...
        const networkError = new Error('Error de conexion. Verifica tu conexion a internet.');
...
/**
 * Funcion de utilidad para hacer requests sin retry automatico
 */
...
/**
 * Funcion de utilidad para verificar la salud del servidor
 */
